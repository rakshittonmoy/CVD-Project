...
def inpaint_image(original_image, masked_image, num_iterations=3000, learning_rate=0.01, save_progress=True):
    """
    ...
    # Resize and convert tensors if not already resized
    resize_transform = transforms.Resize((256, 256))
    original_image = resize_transform(original_image)
    masked_image = resize_transform(masked_image)

    image_tensor = original_image.to(device)
    mask_tensor = masked_image.to(device)
    ...
    for i in range(num_iterations):
        ...
        loss = torch.mean((out - image_tensor) ** 2 * mask_tensor)
        loss.backward()
        ...
        #     # Push intermediate results to the queue for GUI update
        #     data_queue.put((out.detach().cpu().squeeze().permute(1, 2, 0).numpy(), f'Iteration {i}, Loss: {loss.item():.6f}'))
...
if __name__ == "__main__":
    ...
    from deep_image_prior import DeepImagePrior, inpaint_image1, save_result
    import os
    ...
    def update_gui():
        ...
        while True:
            try:
                data, title = data_queue.get(timeout=10)  # Adjust timeout as needed
                ax.imshow(data)
                ax.set_title(title)
                plt.draw()
                plt.pause(0.1)
            except queue.Empty:
                print("No new data to display or worker thread is finished.")
                break
        plt.ioff()
        plt.show()
